...
export function Hero({ scrollY, componentScale, onScrollTo }: Props) {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-linear-to-b from-background/5 via-background/10 to-background/30 z-20" />
      <div className="relative z-40 max-w-4xl mx-auto text-center">
        <div className="mb-8">
          <h1 className="text-4xl sm:text-5xl md:text-7xl font-bold mb-6 text-balance animate-pulse font-sans leading-tight">{HERO.title}</h1>
          <p className="text-lg sm:text-xl md:text-2xl mb-8 text-pretty backdrop-blur-sm bg-background/20 rounded-lg p-3 sm:p-4 font-mono">{HERO.subtitle}</p>
